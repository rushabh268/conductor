...
Source compatibility failures appear in **Connections** as reader health. They do not change retained native session lifecycle. **Attention** shows native waiting and error states only.

Native Codex and OpenCode archives are retained in the local index for identity reconciliation but excluded from visible lists, counts, activity, and usage. Descendant usage stops at archived or locally hidden branches. Native unarchive restores visibility unless the user also hid that session in Conductor; pins and local hiding survive reconciliation. Schemas without an archive field remain supported and do not imply an archive state.
